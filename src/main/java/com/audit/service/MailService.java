...
public class MailService {
    ...
    { 
    	log.debug("Sending audit plan created email to '{}'", auditplan.getAssignTo().getEmail());
    	Locale locale = Locale.forLanguageTag(user.getLangKey());
    	Context context = new Context(locale);
    	context.setVariable("auditplan", auditplan);
    	context.setVariable("createdUser", user);
    	String content = templateEngine.process("mail/auditPlanAssigned.html", context);
    	String subject = "Audit Plan assigned notification";
    	sendEmail(auditplan.getAssignTo().getEmail(), subject, content, false, true);
    }
